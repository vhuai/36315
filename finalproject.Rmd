...
# Data (NA Filtered)
sleep_clean <- sleep[!rowSums(is.na(sleep)), ]
...
# Quantitative Variables (NA Filtered)
sleep_quant <- sleep_clean %>%
...
**What is the impact of sleep patterns on GPA?**
...
**What is the impact of being a first-generation or underrepresented student on academic outcomes? Does gender play a role in academic success?**
...
**Do students from different schools experience different levels of sleep or academic performance?**
...
We initially explore the relationships between sleep and academic success.
...
# Create PCA
sleep_pca <- sleep_clean %>%
  dplyr::select(c(TotalSleepTime, midpoint_sleep, cum_gpa, Zterm_units_ZofZ)) %>%
...
sleep_pca %>%
  fviz_pca_biplot(label="var", alpha.ind=0.3, 
                  alpha.var=0.8, col.var="darkblue")
...
# First-Generation, Underrepresentation and Gender in Academic Success
